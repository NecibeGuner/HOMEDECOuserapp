...
import com.necibeguner.homedeco.DecoHomeApplication
...
class UserAccountViewModel @Inject constructor(
    ...
    fun updateUser(user: User, imageUri: Uri?){
        ...
        if (!areInputsValid){
            viewModelScope.launch {
                _user.emit(Resource.Error("Bilgilerinizi Kontrol Edin"))
            }
            return
        }
    ...
    private fun saveUserInformationWithNewImage(user: User, imageUri: Uri) {
        viewModelScope.launch {
            try {
                val imageBitmap = MediaStore.Images.Media.getBitmap(getApplication<DecoHomeApplication>()
